...
javascript:(function(){
  ...
  const getValueFromList =  (l, type, valueIndex) => { for(let i = 0; i < l.length; i++) { let elem = l[i]; if(elem.children[0].innerText == type) { return elem.childNodes[valueIndex]; } } };  
  ...
  for(let i=0; i < details.length; i++) {
    let ul = details[i];
    let children = ul.getElementsByTagName('li');  ul.append(createLi(getMP(children))); 
  }
})();
